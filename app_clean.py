...
with st.sidebar:
    ...
    if st.session_state.current_user == "binhpt5":
        st.markdown("---")
        ...
        if st.button("🚀 Lấy Báo Cáo Chi Tiết TTHT Xử Lý", use_container_width=True):
            try:
                import requests
                requests.post("https://ntfy.sh/vnpt_scraper_trigger_b8f2d9a74c1e9x3q", data="RUN_TTHT".encode(encoding='utf-8'))
                st.toast("✅ Đã gửi lệnh lấy Báo cáo TTHT xuống máy tính!")
            except Exception as e:
                st.error(f"Lỗi gửi lệnh: {e}")
...
def render_team_detail():
    ...
    if st.session_state.selected_metric == 'brcd':
        ...
        team_df['Ty_Le_Dat'] = (team_df['SM3'] / team_df['SM4'] * 100).fillna(0)
        ...
        display_df = pd.DataFrame({
            ...
            'Tỷ lệ đạt (%)': team_df['Ty_Le_Dat'].apply(lambda x: f"{x:.2f}%")
        })
