...
def process_ttht_excel(file_path, date_str):
    ...
    try:
        ...
        for idx, row in df_merged.iterrows():
            ...
            dat_ko_hen = int(row.get('DAT_KO_HEN', 0))
        ...
        for ma_nv_extracted, data in nvkt_mapping.items():
            ...
            cursor.execute("""
                UPDATE kpi_daily SET SM4_Dat_HT=?, SM4_Khong_Dat_HT=?, SM4_Dat_HT_In_SM3=?
                WHERE Ngay_Bao_Cao=? AND Ma_NV=?
            """, (data['dat_ht'], data['khong_dat_ht'], data['dat_ht_in_sm3'], date_str, ma_nv_extracted))
            
        cursor.execute("""
            UPDATE kpi_daily SET SM4_Dat_HT=?, SM4_Khong_Dat_HT=?, SM4_Dat_HT_In_SM3=?
            WHERE Ngay_Bao_Cao=? AND Ma_NV='TỔNG'
        """, (total_file_dat_ht, total_file_khong_dat_ht, total_file_dat_ht_in_sm3, date_str))
            
        conn.commit()
        conn.close()
        
        logging.info(f"Đã xử lý file TTHT, cập nhật giảm trừ HT cho {len(nvkt_mapping)} NVKT (Đạt HT: {total_file_dat_ht} trong đó có {total_file_dat_ht_in_sm3} phiếu đã Đạt SM4, Không Đạt HT: {total_file_khong_dat_ht}) cho ngày {date_str}")
