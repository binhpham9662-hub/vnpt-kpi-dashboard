...
def run_download_ttht():
    import glob
    import os
    import re
    DOWNLOAD_DIR = "downloads"
    logging.info("Dọn dẹp file SM1 cũ...")
    for f in glob.glob(os.path.join(DOWNLOAD_DIR, "TTHT_*.xlsx")):
        try:
            os.remove(f)
        except:
            pass
            
    logging.info("Bắt đầu quá trình tải báo cáo TTHT xử lý...")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            file_name = f"TTHT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            file_path = os.path.join(DOWNLOAD_DIR, file_name)
            download.save_as(file_path)
            logging.info(f"✅ Đã tải thành công file Excel: {file_path}")
            
            try: bot.send_message(CHAT_ID, f"✅ Đã tải thành công báo cáo TTHT xử lý: {file_name}")
            except: pass
            
            from database import process_ttht_excel
            today_str = datetime.now().strftime("%Y-%m-%d")
            process_ttht_excel(file_path, today_str)
                ...
                for line in resp.iter_lines():
                    if line:
                        ...
                        if data.get('event') == 'message':
                            ...
                            if msg in ["RUN_KPI", "RUN_SM1", "RUN_SM4", "RUN_TTHT"]:
                                task_queue.put(msg)
            except Exception as e:
